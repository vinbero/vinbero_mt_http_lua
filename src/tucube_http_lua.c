...
int tucube_epoll_http_module_onGetRequestContentLength(struct tucube_module* module, struct tucube_cldata* cldata, ssize_t* content_length) {
    ...
    lua_pushinteger(tlmodule->L, *GONC_CAST(cldata->pointer, struct tucube_http_lua_cldata*)->client_socket); // onGetRequestContentLength requests client_socket
    ...
    *content_length = lua_tointeger(tlmodule->L, -1); // requestContentLength
    lua_pop(tlmodule->L, 1); //
    return 0;
}
...
int tucube_epoll_http_module_onRequestBody(struct tucube_module* module, struct tucube_cldata* cldata, char* bodyChunk, ssize_t bodyChunkSize) {
    ...
    lua_pushinteger(tlmodule->L, *GONC_CAST(cldata->pointer, struct tucube_http_lua_cldata*)->client_socket); // onRequestBody requests client_socket
